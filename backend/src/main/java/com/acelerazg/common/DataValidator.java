...
public class DataValidator {
    ...
    public static Response<Void> validateCreateTaskData(String name, String description, String category, Status status, int priority, LocalDateTime endDate) {
    ...
    public static Response<Void> validateUpdateTaskData(String name, int priority, LocalDateTime endDate) {
        if (isNullOrEmpty(name)) return Response.error(400, Messages.ERROR_EMPTY_NAME);
        if (priority < 1 || priority > 5) return Response.error(422, Messages.ERROR_PRIORITY_RANGE);
        if (endDate != null && endDate.isBefore(LocalDateTime.now()))
            return Response.error(422, Messages.ERROR_END_DATE_PAST);
